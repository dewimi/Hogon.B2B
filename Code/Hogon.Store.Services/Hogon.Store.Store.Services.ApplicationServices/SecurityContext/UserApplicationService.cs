    ...
    public class UserApplicationService : BaseApplicationService
    {
        ...
        public void SaveUser(DtoUser dtoUser)
        {
            if (dtoUser.Id == new Guid())
            {
               ...
               var isExist = _userReps.FindBy(m => m.Name == dtoUser.Name).FirstOrDefault();
                if (isExist == null)
                {
                    // 如果密码是新密码，对密码进行加密
            ...
            {
                var user = _userReps.FindBy(i => i.Id == dtoUser.Id).First();
        ...
        {
            int result = 0;
            var user = _userReps.FindBy(i => i.Name == userName).FirstOrDefault();
        ...
        /// </summary>
        /// <returns></returns>
        public DtoUser GetUserById(Guid Id)
        {
            var user = _userReps.FindBy(i => i.Id == Id).First();
        ...
        private User ValidateUser(string userName, string password)
        {
            ...
            // 使用加密后的密码进行比较  
            var currentUser = _userReps.Include("Rela_Role_User.Role.Authorities.Menu.Functions")
        ...
        {
            var role = _userReps.FindBy(r => r.Id == userId).SelectMany
